...
```{r}
polygon2 <- function(x, y1, y2, col = 4, alpha = .1, ...) {
...
```{r}
# chunk to be removed at some point

#uptake <- function(d, Y, U, u) {
#  (U^u + (1 - Y) * d^u) / (U^u + d^u)
#}

uptake <- function(d, Y, U, u) 1 - hill(d, U, Y, u)
```
...
Xm <- Xm_prop * proposed_duration
ts <- seq2(0, proposed_duration)
...
legend2("topleft", legend = c("fatigue", "mitigation", "dropping probability"),
        col = c(3, 2, 4))
```
...
probabilities <- function(pd = 40,
  ...
  if (lgnd)
    legend2("topleft",
            legend = c("fatigue", "mitigation", "dropping probability"),
            col = c(3, 2, 4))
...
```{r}
dropping_probability <- function(pd = 40,
                                 Xf = 25, Yf = .8, hf = 7,
                                 Xmp = .875, Ym = 1, hm = 50,
                                 by = pd / (le - 1), le = 512) {
  ts <- seq(0, pd, by)
...
```{r}
effective_duration <- function(pd = 40,
                               Xf = 25, Yf = .8, hf = 7,
                               Xmp = .875, Ym = 1, hm = 50,
                               by = pd / (le - 1), le = 512) {
...
plot5 <- function(...) plot2(..., xlab = "actual duration of treatment (days)")
...
with(dropping_probability(pd, Xf, Yf, hf, Xmp, Ym, hm, by, le),
     plot4(x, y, ylab = "probability of dropping treatment"))

with(effective_duration(pd, Xf, Yf, hf, Xmp, Ym, hm, by, le), {
  plot5(x, y, ylab = "probability density")
  polygon3(x, y)
})
...
$$
where $d$ is the prescribed duration of treatment. Given that a person can drop
for one of the two reason, the probability of dropping simply reads

$$
p(t, d) = p_1(t) + p_2(t) - p_1(t) \times p_2(t)
$$
...
Probability of dropping the treatment due to some fatigue effect:
...
plot(NA, xlim = c(0, max(ts)), ylim = 0:1, xlab = "time in treatment (days)",
     ylab = "probability")
h1_val <- exp(seq(-3, 3))
abline2(v = K1, col = "grey")
abline2(h = theta1, col = "grey")
walk2(h1_val, rev(RColorBrewer::brewer.pal(n = length(h1_val), "Spectral")),
      ~ lines2(ts, p1(ts, theta1, h1 = .x, K1), col = .y))
```

Mitigation of the probability of dropping the treatment due to some
*"finish line"* effect:

```{r}
p2 <- function(t, theta2, h2, K2) {
  1 - theta2 * t^h2 / (K2^h2 + t^h2)
}
```
...
```{r}
ts <- seq2(0, 40)
plot2(ts, p2(ts, theta2 = 1, h2 = 50, K2 = 35), xlim = c(0, max(ts)), ylim = 0:1,
      xlab = "time in treatment (days)", ylab = "mitigation factor")
```
...
durations <- function(d, theta1, theta2, h1, h2, K1, K2, le = 512) {
  xs <- seq(0, d, le = le)
  prob1 <- p1(xs, theta1, h1, K1)
  prob2 <- p2(xs, d, theta2, h2, K2)
  x <- prob1 + prob2 - prob1 * prob2
...
plot5 <- function(...) plot2(..., xlab = "actual duration of treatment (days)")
legend3 <- function(...) {
  legend2(..., legend = c("fatigue", "finish line", "any cause"),
          col = c(4, 2, 3))
}

prob1 <- p1(ts, theta1, h1, K1)
prob2 <- p2(ts, treatment_duration, theta2, h2, K2)

opar <- par(mfrow = c(1, 3))

plot4(ds, prob1,
      ylab = "probability of dropping treatment")
lines2(ds, prob2)
lines2(ds, prob1 + prob2 - prob1 * prob2, col = 3)
legend3("topleft")

plot4(ds, prob2, col = 2, ylim = c(0, .0003),
      ylab = "probability of dropping treatment")
lines2(ds, prob1, col = 4)
lines2(ds, prob1 + prob2 - prob1 * prob2, col = 3)
legend3("topright")
...
```{r}
durations2 <- function(d, theta1, theta2, h1, h2, K1, K2, by = .05) {
  xs <- seq(0, d, by)
  prob1 <- p1(xs, theta1, h1, K1)
  prob2 <- p2(xs, d, theta2, h2, K2)
  x <- prob1 + prob2 - prob1 * prob2
  out <- c(x[1], x[-1] * cumprod(1 - x[-length(xs)]))
  tibble(x = xs,
         y = out / sum(out))
}
```
...
```{r}
with(durations2(10, theta1, theta2, h1, h2, K1, K2), {
  plot5(x, y, ylab = "probability density", xlim = c(0, 30))
  polygon3(x, y, col = 4)
})

with(durations2(20, theta1, theta2, h1, h2, K1, K2), {
  lines2(x, y, col = 3)
  polygon3(x, y, col = 3)
})

with(durations2(30, theta1, theta2, h1, h2, K1, K2), {
  lines2(x, y, col = 2)
  polygon3(x, y, col = 2)
})

legend2("topright", legend = paste0(10 * 1:3, " days proposed"), col = 4:2)
```
...
walk(10 * 2:10,
     ~ with(durations2(.x, theta1, theta2, h1, h2, K1, K2), {
         lines2(x, y, col = 4)
         polygon3(x, y)
       })
)
...
```{r}
prob_any <- function(treatment_duration, ts, theta1, h1, K1, theta2, h2, K2) {
  prob1 <- p1(ts, theta1, h1, K1)
  prob2 <- p2(ts, treatment_duration, theta2, h2, K2)
...
```{r}
xs <- seq2(0, 100)
plot4(xs, prob_any(treatment_duration = 30, ts, theta1, h1, K1, theta2, h2, K2),
      ylab = "probability of dropping treatment")
walk(9:29, ~ lines(xs, prob_any(treatment_duration = .x, ts, theta1, h1, K1,
                                 theta2, h2, K2), col = 4))
```
...
#### 5.4.1 Mean-field model prediction
...
xs <- seq2(0, 100)
ys <- map_dbl(xs,
              ~ tb_cases(treatment_duration = .x, h1 = 5, K1 = 50, theta2 = .0003))
...
#### 5.4.2 Including the distribution of effective treatment time

A function that converts a duration into incidence:
...
duration2incidence <- function(
  ...
  chi <- tau * uptake(d, Y, U, u) * efficiency(d, xi, k, H)
  pis <- chi * (sigma + gamma) / (1 - chi)
  d_star(gamma, sigma, delta, beta, q, pis, N)
}
```

A new version of the `tb_cases()` function that integrates the distribution of
the effective durations of treatment:

```{r}
tb_cases_dstr <- function(
    tau = .45, # probable range between 30 and 60%
    treatment_duration = 30,
    theta1 = .8,
    h1 = 3,
  ...
  phis <- durations(treatment_duration, theta1, theta2, h1, h2, K1, K2, le)
  
  map_dbl(phis$x[draw(phis$y, n)],
          ~ duration2incidence(.x, tau, Y, U, u, xi, k, H, sigma, gamma, delta,
                               beta, q, N))
}
```

Generating the data, takes about 30":

```{r eval = FALSE}
xs <- seq2(0, 100)[-1]
ys <- mclapply(xs,
               function(x)
                 tb_cases_dstr(treatment_duration = x, h1 = 5, K1 = 50, theta2 = .0003, n = 1e4),
...
qtls <- map_dfr(ys, ~ quantile(.x, c(tmp, .5, rev(1 - tmp))))

plot2(NA, xlab = "duration of proposed prophylactic treatment (days)",
      ylab = "TB prevalence (per 100,000)",
      xlim = c(0, max(xs)),
      ylim = c(0, max(qtls[, ncol(qtls)], na.rm = TRUE)))

le <- length(cis)
nbcol <- 2 * le + 2
walk(seq_along(cis), ~ polygon2(xs, qtls[[.x]], qtls[[nbcol - .x]]))

lines2(xs, qtls[[le + 1]], col = 4)

xs2 <- seq2(0, 100)
ys2 <- map_dbl(xs2, ~ tb_cases(treatment_duration = .x, h1 = 5, K1 = 50, theta2 = .0003))
lines2(xs2, ys2)
```
