...
### Including the distribution of effective treatment time
...
```{r eval = F, include=F}
xs <- seq2(0, 100)[-1]
ys <- mclapply(xs, function(x) tb_cases_dstr(treatment_duration = x),
...
```{r eval = F, include=F}
qtls <- map_dfr(ys, ~ quantile(.x, c(.025, .05, .1, .5, .9, .95, .975)))

plot2(NA, xlab = "duration of proposed prophylactic treatment (days)",
      ylab = "TB prevalence (per 100,000)",
      xlim = c(0, max(xs)),
      ylim = c(0, max(qtls$`97.5%`, na.rm = TRUE)))

polygon2(xs, qtls$`2.5%`, qtls$`97.5%`)
polygon2(xs, qtls$`5%`, qtls$`95%`)
polygon2(xs, qtls$`10%`, qtls$`90%`)
lines2(xs, qtls$`50%`, col = 4)
