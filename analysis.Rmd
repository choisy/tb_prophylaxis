...
$$
\chi(d) = \tau \times \upsilon(d) \times \varepsilon(\rho(d)) 
$$
...
* $\rho(d)$ is the effective treatment duration as a function of the proposed
treatment duration $d$;
* $\varepsilon(\rho(d))$ is the treatment efficacy as a function of the
effective treatment duration $\rho(d)$.
...
```{r}
ys2 <- xs |>
  mclapply2(function(x) quantile(tb_cases(x, tau = .3), probs)) |> 
  bind_rows()
```

```{r}
ys3 <- xs |>
  mclapply2(function(x) quantile(tb_cases(x, tau = .6), probs)) |> 
  bind_rows()
```
...
walk(1:i, ~ polygon2(xs, ys2[[.x]], ys2[[nbcol - .x]], col = 2))
lines(xs, ys2[[i + 1]], col = 2)

walk(1:i, ~ polygon2(xs, ys3[[.x]], ys3[[nbcol - .x]], col = 3))
lines(xs, ys3[[i + 1]], col = 3)

legend2("right", title = "contact tracing efficacies:",
        legend = c("30 %", "45 %", "60 %"), col = c(2, 4, 3))
```
