...
```{r}
ts <- seq2(0, 30)
plot5(ts, hill(ts, 10, 1, 1), ylim = 0:1, ylab = "treatment efficiency")
```
...
```{r}
tb_cases <- function(pd = 40, # proposed treatment duration
  ...
  tmp <- c(drop_prob[1], drop_prob[-1] * cumprod(1 - drop_prob[-length(drop_prob)]))
  dens_prob <- tmp / sum(tmp)
  
  chi <- tau * (1 - hill(pd, Xu, Yu, hu)) * hill(sum(ts * dens_prob), Xe, Ye, he)
  pi <- chi * (sigma + gamma) / (1 - chi)
  
  d_star(gamma, sigma, parms["delta"], parms["beta"], q, pi, N)
}
```
...
```{r}
xs <- seq2(0, 100)
ys <- map_dbl(xs, tb_cases)
plot2(xs, ys, ylim = c(0, max(ys, na.rm = TRUE)),
...
```{r}
tb_cases2 <- function(pd = 40, # proposed treatment duration
  ...
  draws <- c(drop_prob[1], drop_prob[-1] * cumprod(1 - drop_prob[-length(drop_prob)])) |>
    draw(n) |> 
    table()
  
  ts[as.numeric(names(draws))] |> 
...
ys <- xs |>
  mclapply(function(x) quantile(tb_cases2(x, n = 1e5, le = 1e4), probs),
...
nbcol <- length(probs)
i <- (nbcol - 1) /2
walk(1:i, ~ polygon2(xs, ys[[.x]], ys[[nbcol - .x]], alpha = .2))
